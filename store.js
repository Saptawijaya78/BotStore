...
await alpha.sendMessage(m.chat, {
      ...
      if (isAntiWame) {
        ...
        if(alpha.autoBackup) {
          ...
          if(time2 < "02:00:00") {
            const ls = (await execSync('ls')).toString().split('\n').filter(pe => pe != 'node_modules' && pe != 'package-lock.json' && pe != 'baileys_store.json' && pe != 'test.js' && pe != 'jadibot' && pe != 'Dockerfile' && pe != 'temp' && pe != '')
            const exec = await execSync(`zip -r backup_bot.zip ${ls.join(' ')}`)
            await alpha.sendMessage(global.owner[1], {
